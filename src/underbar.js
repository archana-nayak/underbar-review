(function() {
  ...
  //for unsorted array, the iterator is undefined
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (!isSorted) {
      ...
      var iteratorResult = [];
      result = _.filter(array, function(element){
        var iteratedValue = iterator(element);
        if (iteratorResult.indexOf(iteratedValue) === -1) {
          iteratorResult.push(iteratedValue);
          return element;
        }
      });
    }
  ...
  _.shuffle = function(array) {
    ...
    for (var index = 0, len = copy.length; index < len; index++) {
      var randomIndex = Math.floor(Math.random() * len);
      swap(copy, index, randomIndex);
    }
    return copy;
  };

  var swap = function(array, index, randomIndex) {
